...
async function findUser(username) {
    ...
    const [rows] = await conn.query(`SELECT * FROM funcionarios WHERE email=? LIMIT 1`, [username]);
    if (rows.length > 0)
        return rows[0];
    else return null;
}
...
async function findUserById(id) {
    const conn = await connect();
    const [rows] = await conn.query(`SELECT * FROM funcionarios WHERE id_funcionario=? LIMIT 1`, [id]);
    if (rows.length > 0)
        return rows[0];
    else return null;
}
